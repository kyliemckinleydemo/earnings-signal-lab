...
=========================================================
Pulls real earnings transcripts from FMP, analyzes them with Claude API
for 16 granular NLP features, and backtests against real price data.

Requirements:
    pip install requests yfinance pandas numpy anthropic scipy

Setup:
    1. Get a free FMP API key at https://financialmodelingprep.com/developer
...
FMP_API_KEY = os.environ.get("FMP_API_KEY", "YOUR_FMP_KEY_HERE")
...
# ============================================================
# STEP 1: Pull Transcripts from FMP
# ============================================================
...
def get_transcript_dates(symbol: str) -> list[dict]:
    ...
    try:
        ...
        print(f"  Error getting dates for {symbol}: {e}")
        return []


def get_transcript(symbol: str, quarter: int, year: int) -> dict | None:
    ...
    try:
        ...
        print(f"  Error getting transcript {symbol} Q{quarter} {year}: {e}")
        return None


def pull_all_transcripts():
    """Pull transcripts for all companies, respecting rate limits."""
    print("=" * 60)
    print("STEP 1: Pulling Earnings Transcripts from FMP")
    print("=" * 60)
    ...
    for symbol in COMPANIES:
        print(f"\n[{symbol}] Getting transcript dates...")
        ...
        dates = get_transcript_dates(symbol)
        api_calls += 1
        time.sleep(0.15)  # Rate limit: ~6/sec to stay safe under 10/sec
        ...
        recent = dates[:QUARTERS_BACK]
        print(f"  Found {len(dates)} dates, pulling most recent {len(recent)}")
        ...
        for entry in recent:
            ...
            if api_calls >= 240:  # Leave buffer under 250
                print("\n⚠️  Approaching FMP daily limit (250 calls). Stopping.")
                print("  Re-run tomorrow to continue pulling transcripts.")
                _save_transcripts(cache_file, existing)
                return total_pulled

            transcript = get_transcript(symbol, q, y)
            api_calls += 1
            time.sleep(0.15)
            ...
            if transcript:
                ...
                total_pulled += 1
                print(f"  ✓ Q{q} {y} ({len(transcript.get('content', ''))} chars)")
            else:
                print(f"  ✗ Q{q} {y} - no data")

        _save_transcripts(cache_file, existing)

    print(f"\nDone: {total_pulled} new transcripts pulled, {total_skipped} cached, {api_calls} API calls used")
    return total_pulled
...
def main():
    ...
    needs_fmp = args.step in [None, "pull", "prices"] or args.refresh or args.refresh_all
    needs_claude = args.step in [None, "analyze"]

    if needs_fmp and FMP_API_KEY == "YOUR_FMP_KEY_HERE":
        print("\n⚠️  Set FMP_API_KEY environment variable or edit this script.")
